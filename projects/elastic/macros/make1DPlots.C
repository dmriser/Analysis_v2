{
  ...
  //   User Parameters 
  // --------------------------------------
  TFile *inputFile = TFile::Open("../phiBins1thetaBins50noSin.root"); 
  string imagePath = "/volatile/clas12/dmriser/plots/elastic/t50p1noSin/";
  ...
  for(int s=0; s<numberSectors; s++){
    for(int b=0; b<numberPhiBins; b++){
      ...
      stats.DrawLatex(0.75, 0.9, Form("#splitline{Sector %d}{N_{Events} = %3.E}",s+1,data[s][b]->Integral())); 
